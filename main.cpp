...
{
    while (true) {
        int potValue = uBit.io.P0.getAnalogValue(); // Read the potentiometer value (0 to 1023)
        int toneFrequency = potValue * 1000 / 1023; // Map to frequency range (0 to 1000 Hz)

        if (toneFrequency > 0) {
            uBit.io.P15.setAnalogValue(512); // Set PWM for buzzer
            uBit.io.P15.setAnalogPeriodUs(1000000 / toneFrequency); // Set tone frequency
        } else {
            uBit.io.P15.setAnalogValue(0); // Turn off the buzzer
        }

        uBit.sleep(100); // Add a small delay for stability
    }
}
...
{
    while (true) {
        uBit.io.P14.setDigitalValue(1); // Turn the LED ON
        uBit.sleep(500);               // Wait 500ms
        uBit.io.P14.setDigitalValue(0); // Turn the LED OFF
        uBit.sleep(500);               // Wait 500ms
    }
}
...
void led_bulb()
{
    while (true) {
        if (uBit.buttonB.isPressed()) {
            uBit.io.P13.setDigitalValue(1); // Turn the LED ON
        } else {
            uBit.io.P13.setDigitalValue(0); // Turn the LED OFF
        }
        uBit.sleep(100); // Add a small delay for stability
    }
}
